...
const getCouleur = (matiere) => {
  ...
  let hash = 0;
  for (let i = 0; i < (matiere?.length || 0); i++) {
  ...
  const handleCreerCreneau = async () => {
    ...
    try {
      ...
      if (res.data.succes) {
        ...
        setShowModal(false);
        setForm({ id_classe: classeId, id_matiere: "", id_enseignant: "", id_salle: "", jour: "Lundi", heure_debut: "08:00", heure_fin: "10:00" });
        chargerPlannings(classeId);
      } else {
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: bg, transition: "all 0.3s" }}>
      ...
      <div style={{ width: sidebarOpen ? "220px" : "60px", background: "#04342C", transition: "width 0.3s", display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, padding: "8px" }}>
          {menuItems.map(item => (
            <div key={item.label} onClick={() => navigate(item.route)} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 8px", borderRadius: "8px", cursor: "pointer", background: item.active ? "#0F6E56" : "transparent", marginBottom: "4px" }}>
  ...
  Semaine du {new Date(semaineDebut).toLocaleDateString("fr-FR", { day: "numeric", month: "long" })} au {new Date(new Date(semaineDebut).setDate(new Date(semaineDebut).getDate() + 5)).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
            {message && (
              <div style={{ padding: "6px 12px", background: message.includes("✅") || message.includes("📢") ? "#E1F5EE" : message.includes("⚠️") ? "#FAEEDA" : "#FCEBEB", color: message.includes("✅") || message.includes("📢") ? "#085041" : message.includes("⚠️") ? "#633806" : "#791F1F", borderRadius: "8px", fontSize: "12px", fontWeight: "500" }}>
                {message}
              </div>
            )}
            ...
            <div style={{ display: "flex", background: bg3, borderRadius: "8px", overflow: "hidden" }}>
              {[{ val: "semaine", label: "📅 Semaine" }, { val: "journee", label: "📆 Journée" }, { val: "liste", label: "📋 Liste" }].map(v => (
                <button key={v.val} onClick={() => setVue(v.val)} style={{ padding: "6px 12px", border: "none", cursor: "pointer", fontSize: "11px", background: vue === v.val ? "#0F6E56" : "transparent", color: vue === v.val ? "#fff" : txt2, fontWeight: vue === v.val ? "500" : "400" }}>{v.label}</button>
              ))}
            </div>
...
<button onClick={() => {
  ...
  fenetre.print();
        ...
        {/* Statut publication */}
        {plannings.length > 0 && (
          <div style={{ padding: "8px 20px", background: plannings[0].statut_publication === "publie" ? "#E1F5EE" : "#FAEEDA", borderBottom: `0.5px solid ${brd}`, display: "flex", alignItems: "center", gap: "8px" }}>
            <span>{plannings[0].statut_publication === "publie" ? "📢" : "📄"}</span>
  ...
  {new Date(new Date(semaineDebut).setDate(new Date(semaineDebut).getDate() + i)).toLocaleDateString("fr-FR", { day: "numeric", month: "short" })}
                      ...
                      return (
                        <div key={jour} style={{ background: bg2, borderRadius: "8px", border: `0.5px solid ${brd}`, minHeight: "90px", padding: "3px" }}>
                          {crs.map((cr, i) => {
                            ...
                            return (
                              <div key={i} onClick={() => setCreneauSelec(cr)} style={{ background: c.bg, borderLeft: `3px solid ${c.border}`, borderRadius: "4px", padding: "6px 8px", cursor: "pointer", height: "calc(100% - 6px)" }}>
                                <p style={{ fontSize: "11px", fontWeight: "500", color: c.txt, margin: "0 0 3px" }}>{cr.matiere?.split(" ").slice(0, 2).join(" ")}</p>
                                <p style={{ fontSize: "10px", color: c.txt, margin: "0 0 4px", opacity: 0.8 }}>{cr.enseignant?.split(" ").slice(-1)[0]}</p>
                                <span style={{ fontSize: "9px", background: c.border, color: "#fff", padding: "1px 5px", borderRadius: "8px" }}>{cr.salle}</span>
                              </div>
                            );
                          })}
                        </div>
            ...
            <div>
              <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap" }}>
                {JOURS.map((jour, i) => (
                  <button key={jour} onClick={() => setJourSelec(jour)} style={{ padding: "8px 16px", borderRadius: "8px", border: `0.5px solid ${jourSelec === jour ? "#0F6E56" : brd}`, cursor: "pointer", background: jourSelec === jour ? "#0F6E56" : bg2, color: jourSelec === jour ? "#fff" : txt, fontSize: "12px", fontWeight: jourSelec === jour ? "500" : "400" }}>
                    {jour} {14 + i} avr.
                  </button>
                ))}
              </div>
              <div style={{ background: bg2, borderRadius: "12px", border: `0.5px solid ${brd}`, overflow: "hidden" }}>
                <div style={{ padding: "14px 16px", borderBottom: `0.5px solid ${brd}`, background: bg3 }}>
                  <p style={{ fontSize: "14px", fontWeight: "500", color: txt, margin: 0 }}>📆 {jourSelec} — {getCreneauxJour(jourSelec).length} créneau(x)</p>
                </div>
                {getCreneauxJour(jourSelec).length === 0 ? (
                  ...
                  getCreneauxJour(jourSelec).map((cr, i) => {
                    const c = getCouleur(cr.matiere);
                    return (
                      <div key={i} onClick={() => setCreneauSelec(cr)} style={{ display: "flex", gap: "16px", alignItems: "stretch", padding: "16px", borderBottom: `0.5px solid ${brd}`, cursor: "pointer" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "6px", justifyContent: "center" }}>
                          <button onClick={(e) => { e.stopPropagation(); setQrCreneau(cr); setShowQR(true); }} style={{ padding: "6px 12px", background: "#E1F5EE", color: "#085041", border: "0.5px solid #9FE1CB", borderRadius: "6px", fontSize: "11px", cursor: "pointer", fontWeight: "500" }}>📱 QR-Code</button>
                        </div>
                      </div>
                    );
                  })
                  ...
                  return (
                    <div key={jour} style={{ marginBottom: "16px" }}>
                      ...
                      {crs.map((cr, i) => {
                        const c = getCouleur(cr.matiere);
                        return (
                          <div key={i} onClick={() => setCreneauSelec(cr)} style={{ background: bg2, borderRadius: "10px", border: `0.5px solid ${brd}`, padding: "12px 16px", display: "flex", alignItems: "center", gap: "16px", cursor: "pointer", marginBottom: "6px", borderLeft: `4px solid ${c.border}` }}>
                            ...
                            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                              ...
                              <button onClick={(e) => { e.stopPropagation(); setQrCreneau(cr); setShowQR(true); }} style={{ padding: "5px 10px", background: "#E1F5EE", color: "#085041", border: "none", borderRadius: "6px", fontSize: "11px", cursor: "pointer" }}>📱 QR</button>
                            </div>
                          </div>
                        );
                      })}
      ...
      {creneauSelec && (
        <div onClick={() => setCreneauSelec(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: bg2, borderRadius: "16px", padding: "1.5rem", width: "380px", border: `0.5px solid ${brd}` }}>
              ...
              return (
                <div>
                  ...
                  {[
                    ...
                    { label: "Token QR",   val: getToken(creneauSelec) },
                  ].map(item => (
                    <div key={item.label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `0.5px solid ${brd}` }}>
                      ...
                      <span style={{ fontSize: "13px", fontWeight: "500", color: item.label === "Token QR" ? "#0F6E56" : txt }}>{item.val}</span>
                    </div>
                  ))}
                  <div style={{ display: "flex", gap: "8px", marginTop: "16px" }}>
                    <button onClick={() => { setQrCreneau(creneauSelec); setShowQR(true); setCreneauSelec(null); }} style={{ flex: 1, padding: "10px", background: "#0F6E56", color: "#fff", border: "none", borderRadius: "8px", fontSize: "13px", cursor: "pointer", fontWeight: "500" }}>📱 Voir QR-Code</button>
                    <button onClick={() => { setForm({ id_classe: classeId, id_matiere: creneauSelec.id_matiere || "", id_enseignant: creneauSelec.id_enseignant || "", id_salle: creneauSelec.id_salle || "", jour: creneauSelec.jour, heure_debut: creneauSelec.heure_debut?.slice(0,5) || "08:00", heure_fin: creneauSelec.heure_fin?.slice(0,5) || "10:00", id_creneau: creneauSelec.id }); setCreneauSelec(null); setShowModal(true); }} style={{ flex: 1, padding: "10px", background: "#FAEEDA", color: "#633806", border: "0.5px solid #E8C97A", borderRadius: "8px", fontSize: "13px", cursor: "pointer", fontWeight: "500" }}>✏️ Modifier</button>
                    <button onClick={async () => { if (!window.confirm("Supprimer ce créneau ?")) return; try { await axios.delete(`${API}/emploi_temps.php?id=${creneauSelec.id}`, { headers: { Authorization: `Bearer ${token}` } }); setCreneauSelec(null); setMessage("✅ Créneau supprimé !"); chargerPlannings(classeId); setTimeout(() => setMessage(""), 3000); } catch (err) { setMessage("❌ Erreur lors de la suppression"); } }} style={{ flex: 1, padding: "10px", background: "#FCEBEB", color: "#791F1F", border: "0.5px solid #F09595", borderRadius: "8px", fontSize: "13px", cursor: "pointer", fontWeight: "500" }}>🗑️ Supprimer</button>
                    <button onClick={() => setCreneauSelec(null)} style={{ flex: 1, padding: "10px", background: bg3, color: txt, border: `0.5px solid ${brd}`, borderRadius: "8px", fontSize: "13px", cursor: "pointer" }}>Fermer</button>
                  </div>
                </div>
              );
            })()}
      ...
      {/* Modal QR-Code */}
      {showQR && qrCreneau && (
        <div onClick={() => setShowQR(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: bg2, borderRadius: "16px", padding: "2rem", width: "340px", textAlign: "center", border: `0.5px solid ${brd}` }}>
            ...
            <div style={{ background: "#E1F5EE", borderRadius: "8px", padding: "12px", marginBottom: "12px", textAlign: "left" }}>
              <p style={{ fontSize: "11px", color: "#5F5E5A", margin: "0 0 6px", fontWeight: "500" }}>🔑 Token de pointage :</p>
              <p style={{ fontSize: "14px", color: "#0F6E56", margin: 0, fontFamily: "monospace", fontWeight: "700", wordBreak: "break-all" }}>
                {getToken(qrCreneau)}
              </p>
            </div>
            <div style={{ background: "#FAEEDA", borderRadius: "8px", padding: "8px 12px", marginBottom: "16px", display: "flex", gap: "8px", alignItems: "center" }}>
              <span>⏰</span>
              <p style={{ fontSize: "12px", color: "#633806", margin: 0 }}>Valide ±15 min autour de {qrCreneau.heure_debut?.slice(0,5)} — Usage unique</p>
            </div>
            <div style={{ display: "flex", gap: "8px" }}>
              <button onClick={() => { navigator.clipboard?.writeText(getToken(qrCreneau)); setMessage("✅ Token copié !"); setTimeout(() => setMessage(""), 2000); setShowQR(false); }} style={{ flex: 1, padding: "10px", background: "#0F6E56", color: "#fff", border: "none", borderRadius: "8px", fontSize: "13px", cursor: "pointer", fontWeight: "500" }}>📋 Copier token</button>
              <button onClick={() => setShowQR(false)} style={{ flex: 1, padding: "10px", background: bg3, color: txt, border: `0.5px solid ${brd}`, borderRadius: "8px", fontSize: "13px", cursor: "pointer" }}>Fermer</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: bg2, borderRadius: "16px", padding: "1.5rem", width: "460px", border: `0.5px solid ${brd}` }}>
            ...
            <div style={{ background: "#FAEEDA", borderRadius: "8px", padding: "8px 12px", marginBottom: "12px", display: "flex", gap: "8px" }}>
              <span>⚠️</span>
              <p style={{ fontSize: "12px", color: "#633806", margin: 0 }}>Les conflits (enseignant déjà occupé, salle prise) seront détectés automatiquement.</p>
            </div>
            <div style={{ display: "flex", gap: "10px" }}>
              <button onClick={handleCreerCreneau} style={{ flex: 1, padding: "11px", background: "#0F6E56", color: "#fff", border: "none", borderRadius: "8px", fontSize: "13px", fontWeight: "500", cursor: "pointer" }}>✅ Créer le créneau</button>
